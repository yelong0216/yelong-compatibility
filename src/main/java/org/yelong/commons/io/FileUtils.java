...
public class FileUtils {
	 ...
	 */
	public static Long getFileCreateTime(String fileAbsolutePath) throws FileNotFoundException {
	...
	/**
	 * 创建文件，如果文件所在目录不存在则一同创建。 如果这个文件已经存在则删除原文件并创建新文件
	 * 
	 * @param names the name elements
	 * @return the file
	 */
	...
	/**
	 * 创建目录。 如果目录存在则返回它本身 如果目录的抽象路径中的目录不存在则创建。 注意：如果目录存在这并不会抛出异常
	 * 
	 * @param names the name elements
	 * @return the file
	 * @throws IOException
	...
	/**
	 * 创建目录。 如果目录存在则删除目录内文件 如果目录的抽象路径中的目录不存在则创建。
	 * 
	 * @param names the name elements
	 * @return the file
	 * @throws IOException
	...
	/**
	 * 清空目录内的所有文件 Cleans a directory without deleting it.
	 * 
	...
	/**
	 * 删除目录(删除目录内的所有文件) Deletes a directory recursively.
	 *
	...
	/**
	 * 安静的删除文件（支持目录、文件） Deletes a file, never throwing an exception. If file is a
	 * directory, delete it and all sub-directories.
	 * <p>
